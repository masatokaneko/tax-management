...
- [ ] InMemoryTransportによるMCPサーバー統合テスト (`test/tools/server.integration.test.ts`)
...
- [ ] `fetch-freee-data` ツール -- freee MCPからBS/PL/仕訳データを一括取得してSQLiteにキャッシュ (`src/tools/data/fetch-freee-data.tool.ts`)
  - 試算表（BS/PL）取得 → 当期純利益の自動取得
  - 仕訳データ取得（ページネーション対応）
  - 勘定科目マスタ取得
  - データの確定状態チェック（月次締め状態の確認）
- [ ] 勘定科目マッピングテーブル -- freee科目名→税務科目名の対応定義 (`src/data/master/account-mapping.json`)
...
- [ ] 別表十六 -- 減価償却資産の計算 (`src/tools/schedules/calculate-schedule-16.tool.ts`)
  - 定額法/定率法の計算
  - 耐用年数別の計算
  - 税務上の償却限度額 vs 会計上の計上額→超過額/不足額
- [ ] 別表十五 -- 交際費等の損金不算入 (`src/tools/schedules/calculate-schedule-15.tool.ts`)
  - 中小法人: 800万円の定額控除限度額
  - 飲食費の50%損金算入特例
- [ ] 別表二 -- 同族会社等の判定 (`src/tools/schedules/calculate-schedule-02.tool.ts`)
  - 株主情報の手動入力
  - 上位3グループの持株比率計算
  - 同族会社/非同族会社の判定
...
- [ ] 別表六 -- 所得税額の控除 (`src/tools/schedules/calculate-schedule-06.tool.ts`)
- [ ] 別表七 -- 欠損金の繰越控除 (`src/tools/schedules/calculate-schedule-07.tool.ts`)
- [ ] 別表八 -- 受取配当等の益金不算入 (`src/tools/schedules/calculate-schedule-08.tool.ts`)
- [ ] 別表十四 -- 寄付金の損金算入 (`src/tools/schedules/calculate-schedule-14.tool.ts`)

#### Phase 3b: 消費税

- [ ] 一般課税（原則課税）の計算 (`src/tools/consumption-tax/calculate-general.tool.ts`)
  - 課税売上に係る消費税額の計算
  - 仕入税額控除の計算（全額控除/個別対応/一括比例配分）
  - インボイス経過措置（80%/50%控除の期間判定）
- [ ] 簡易課税の計算 (`src/tools/consumption-tax/calculate-simplified.tool.ts`)
  - 事業区分ごとのみなし仕入率適用
  - 2種以上の事業がある場合の按分計算

#### Phase 3c: 地方税

- [ ] 法人住民税の計算 (`src/tools/local-tax/calculate-resident-tax.tool.ts`)
  - 法人税割 = 法人税額 × 住民税率
  - 均等割（資本金・従業員数による段階判定）
- [ ] 法人事業税の計算 (`src/tools/local-tax/calculate-enterprise-tax.tool.ts`)
  - 3段階の累進税率（400万/800万/800万超）
  - 東京都超過税率
- [ ] 特別法人事業税 (`src/tools/local-tax/calculate-special-enterprise-tax.tool.ts`)
  - 事業税額 × 37%
...
- [ ] e-Tax仕様書（CAB形式）のダウンロードと解析
- [ ] 別表XMLスキーマ（XSD）の取得と型生成
- [ ] 法人税申告書XML生成 (`src/tools/export/export-etax-xml.tool.ts`)
- [ ] 消費税申告書XML生成
- [ ] 財務諸表XBRL生成（XBRL 2.1形式）

#### Phase 4b: eLTAX XML出力（地方税）

- [ ] eLTAX公開仕様書のダウンロードと解析
- [ ] 法人住民税・事業税XML生成 (`src/tools/export/export-eltax-xml.tool.ts`)
...
- [ ] freeeデータから法人税申告書（別表一〜十六）を自動生成できる
- [ ] 税務調整項目をAI（Claude）が推定し、ユーザーが確認・修正できる
- [ ] 消費税申告書（一般課税・簡易課税）を計算できる
- [ ] 地方税（法人住民税・事業税・特別法人事業税）を計算できる
- [ ] 別表間の整合性チェックが通る（検算式が一致する）
- [ ] e-Tax提出可能なXML/XBRLファイルを出力できる
- [ ] eLTAX提出可能なXMLファイルを出力できる
- [ ] 前期データとの継続性が保たれる（繰越欠損金、利益積立金額等）
- [ ] セッション断絶後に「前回の続きから」で再開できる

### Non-Functional Requirements

- [ ] 税額計算の端数処理が税法の規定に準拠している
- [ ] 全計算過程がSQLiteに記録され、監査時に説明可能
- [ ] SQLiteのWALモードで安定した読み書き性能
- [ ] 金額は全て整数（円単位）で処理し、浮動小数点誤差を排除

### Quality Gates

- [ ] コア別表（一・四・五）の単体テストカバレッジ
- [ ] Scalarの前期申告書データとの突き合わせテストが合格
- [ ] MCP InspectorでのE2Eテスト合格
...
| e-Tax仕様書 | 未ダウンロード | Phase 4の前にダウンロード・解析が必要 |
